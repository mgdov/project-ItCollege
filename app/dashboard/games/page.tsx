...
import { Trophy, Users, Cpu, ArrowRight, Gamepad2, Target } from "lucide-react"
import Link from "next/link"
import Image from "next/image"
import { motion } from "framer-motion"
import { gameStore, useStore } from "@/lib/store"
...
export default function GamesPage() {
  const stats = useStore(gameStore, (state) => state.stats)
  return (
        ...
        <div className="relative px-6 py-12 md:px-8 md:py-16">
          ...
          <div className="mt-6 grid grid-cols-3 gap-4 max-w-2xl">
            ...
            <motion.div
              ...
              className="rounded-xl border border-border/50 bg-background/80 backdrop-blur-sm p-4"
            >
              <div className="text-2xl font-bold text-accent">{stats.gamesPlayed || 0}</div>
            ...
            <motion.div
              ...
              className="rounded-xl border border-border/50 bg-background/80 backdrop-blur-sm p-4"
            >
              <div className="text-2xl font-bold text-warning">{stats.totalScore || 0}</div>
      ...
      >
        <Card className="mt-8 border-border/50 overflow-hidden">
          ...
          <CardContent className="pt-6">
            <EmptyState
              title="Нет данных"
              description="Таблица лидеров появится после подключения к серверу"
            />
          </CardContent>
        </Card>
      </motion.div>
